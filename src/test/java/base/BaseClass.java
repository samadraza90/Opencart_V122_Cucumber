...
public class BaseClass {
	...
	public static void initialSetup() throws MalformedURLException {
		...
		case "remote":
			...
			break;
		case "headless":
			driver = initializeheadlessDriver(os, browser);// headless execution
			break;
		default:
		...
		if (driver != null) {
			...
			if(!executionEnv.contains("headless")) {
	...
	private static WebDriver initializeheadlessDriver(String os,String browser) {
		switch (browser.toLowerCase()) {
		case "chrome": {
			ChromeOptions chromeOptions = new ChromeOptions();
			chromeOptions.addArguments("--headless=new");
